...
//    }
    ...
    if(type == 0){
     Serial.println(F("Searching for JY901 on address 0x50/80"));
      delay(1000);
      if(true){//(JY901.ReadWord(0x1a) == 0x50){
        Serial.println(F("JY901 found on address 0x50"));
        type = 4;
        gyro_address = 0x50;
      }
    }
...
void gyro_signalen(){
  ...
  gyro_roll = (float)JY901.stcGyro.w[1]/32768*2000;
  gyro_pitch = (float)JY901.stcGyro.w[0]/32768*2000;
  gyro_yaw = (float)JY901.stcGyro.w[2]/32768*2000;
}
...
void check_gyro_axes(byte movement){
  ...
  while(timer > millis() && gyro_angle_roll > -30 && gyro_angle_roll < 30 && gyro_angle_pitch > -30 && gyro_angle_pitch < 30 && gyro_angle_yaw > -30 && gyro_angle_yaw < 30){
    ...
    if(type == 4){
      gyro_angle_roll += gyro_roll * 0.004;          // 0.0000611 = 1 / 65.5 (LSB degr/s) / 250(Hz)
      gyro_angle_pitch += gyro_pitch * 0.004;
      gyro_angle_yaw += gyro_yaw * 0.004;
    }
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("JYDrone Setup Program"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));

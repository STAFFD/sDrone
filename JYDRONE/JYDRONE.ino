...
void setup(){
  ...
  if(eeprom_data[31] == 4) JY901.StartIIC();
  else while(true);
  ...
  battery_voltage = (analogRead(A7)) * 1.2317;
...
void loop(){
  ...
  //0.09853 = 0.08 * 1.2317.
  battery_voltage = battery_voltage * 0.92 + (analogRead(A7)) * 0.09853;
...
void gyro_signalen(){
  ...
  } else if(eeprom_data[31] == 4){
    while (Serial.available()) JY901.CopeSerialData(Serial.read()); //Call JY901 data cope function
    gyro_roll_input = (float)JY901.stcGyro.w[1]/32768*2000;
    gyro_pitch_input = (float)JY901.stcGyro.w[0]/32768*2000;
    gyro_yaw_input = -(float)JY901.stcGyro.w[2]/32768*2000;

    angle_roll = (float)JY901.stcAngle.Angle[1]/32768*180 + 2.85;
    angle_pitch = (float)JY901.stcAngle.Angle[0]/32768*180 - 0.18;
//    acc_z = (float)JY901.stcAngle.Angle[2]/32768*180;

//    Serial.print("gyro_roll : ");Serial.print(gyro_roll);

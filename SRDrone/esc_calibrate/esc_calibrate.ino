...
volatile int  receive_channel_previous[6];
...
void loop(){
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      ///We don't want the esc's to be beeping annoyingly. So let's give them a 1000us puls while calibrating the gyro.
      pull_esc(ALL_UP);                                                               //Set digital poort 4, 5, 6 and 7 high.
      delayMicroseconds(1000);                                                          //Wait 1000us.
      pull_esc(ALL_DOWN);                                                               //Set digital poort 4, 5, 6 and 7 low.
...
void esc_pulse_output(){
  ...
  while(PINB & B01100000 || PINC & B01000000 || PIND & B10000000){                                    //Stay in this loop until output 7,8,9 and 10 are low.
    ...
    if(timer_channel_2 <= esc_loop_timer)PORTB &= B11011111;                  //Set digital output 8 to low if the time is expired.
    if(timer_channel_3 <= esc_loop_timer)PORTB &= B10111111;                //Set digital output 9 to low if the time is expired.
    if(timer_channel_4 <= esc_loop_timer)PORTC &= B01111111;                //Set digital output 10 to low if the time is expired.
  }
}

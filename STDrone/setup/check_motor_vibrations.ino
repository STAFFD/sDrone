  ...
  while (data != 'q') {                                                                //Stay in this loop until the data variable data holds a q.
    ...
    if (throttle_init_ok) {                                                            //If the throttle is detected in the lowest position.
      ...
      if (data == '1') {                                                               //If the user requested 1.
        TIMER4_BASE->CCR1 = receiver_input[THROTTLE_INPUT];                                                 //Set the ESC 1 output pulse equal to the throttle input.
      ...
      if (data == '2') {                                                               //If the user requested 2.
        ...
        TIMER4_BASE->CCR2 = receiver_input[THROTTLE_INPUT];                                                 //Set the ESC 2 output pulse equal to the throttle input.
        TIMER4_BASE->CCR3 = 1000;                                                      //Keep the ESC 3 pulse at 1000us.
        TIMER4_BASE->CCR4 = 1000;                                                      //Keep the ESC 4 pulse at 1000us.
      }
      if (data == '3') {                                                               //If the user requested 3.
        TIMER4_BASE->CCR1 = 1000;                                                      //Keep the ESC 1 pulse at 1000us.
        TIMER4_BASE->CCR2 = 1000;                                                      //Keep the ESC 2 pulse at 1000us.
        TIMER4_BASE->CCR3 = receiver_input[THROTTLE_INPUT];                                                 //Set the ESC 3 output pulse equal to the throttle input.
        TIMER4_BASE->CCR4 = 1000;                                                      //Keep the ESC 4 pulse at 1000us.
      }
      if (data == '4') {                                                               //If the user requested 4.
        ...
        TIMER4_BASE->CCR2 = 1000;                                                      //Keep the ESC 2 pulse at 1000us.
        TIMER4_BASE->CCR3 = 1000;                                                      //Keep the ESC 3 pulse at 1000us.
        TIMER4_BASE->CCR4 = receiver_input[THROTTLE_INPUT];                                                 //Set the ESC 4 output pulse equal to the throttle input.
      }
      ...
      if (receiver_input[THROTTLE_INPUT] > 1050) {                                                          //If the trottle channel is not in the lowest position.
      ...
      while (wait_timer > millis() && !throttle_init_ok) {                             //Stay in this wait loop for 10 seconds.
        if (receiver_input[THROTTLE_INPUT] < 1050)throttle_init_ok = 1;                                     //If the throttle is in the lowest position set the throttle_init_ok variable.

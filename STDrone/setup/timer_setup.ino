...
//A test is needed to check if the throttle input is active and valid. Otherwise the ESC's might start without any warning.
  loop_counter = 0;
  while ((receiver_input[THROTTLE_INPUT] > 2100 || receiver_input[THROTTLE_INPUT] < 900) && warning == 0) {
    delay(100);
    loop_counter++;
    if (loop_counter == 40) {
  ...
  if (warning == 0) {
    ...
    TIMER4_BASE->CCR1 = receiver_input[THROTTLE_INPUT];
    TIMER4_BASE->CCR2 = receiver_input[THROTTLE_INPUT];
    TIMER4_BASE->CCR3 = receiver_input[THROTTLE_INPUT];
    TIMER4_BASE->CCR4 = receiver_input[THROTTLE_INPUT];
    pinMode(PB6, PWM);
    pinMode(PB7, PWM);
    pinMode(PB8, PWM);
    pinMode(PB9, PWM);
